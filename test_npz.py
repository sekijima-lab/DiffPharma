...
if __name__ == "__main__":
    ...
    for d in pbar:
        ...
        interh_coords = d['interh_coords'].repeat(args.batch_size,1)
        interh_one_hot = d['interh_one_hot'].repeat(args.batch_size,1)
        interh_mask = torch.repeat_interleave(
            torch.arange(args.batch_size, device=device, dtype=INT_TYPE),len(d['interh_mask'])
        )
        interh_size = torch.tensor([d['num_interh_nodes']]*args.batch_size, device=device, dtype=INT_TYPE)

        # hydrophobic bond information
        interhp_coords = d['interhp_coords'].repeat(args.batch_size,1)
        interhp_one_hot = d['interhp_one_hot'].repeat(args.batch_size,1)
        interhp_mask = torch.repeat_interleave(
            torch.arange(args.batch_size, device=device, dtype=INT_TYPE),len(d['interhp_mask'])
        )
        interhp_size = torch.tensor([d['num_interhp_nodes']]*args.batch_size, device=device, dtype=INT_TYPE)
        ...
        while len(valid_molecules) < args.n_samples:
            ...
            if type(model.ddpm) == ConditionalDDPM: 
                xh_lig, xh_pocket, lig_mask, pocket_mask = \
                    model.ddpm.sample_given_pocket(pocket, interh, interhp, num_nodes_lig,
                                              timesteps=args.timesteps)

            #print('len xh_lig', len(xh_lig))
            #print('xh_lig',xh_lig)
            #break
