...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        self.inth_type_encoder = self.dataset_info['inth_encoder']
        self.inth_type_decoder = self.dataset_info['inth_decoder']
        self.inthp_type_encoder = self.dataset_info['inthp_encoder']
        self.inthp_type_decoder = self.dataset_info['inthp_decoder']
        '''
        self.pocket_type_encoder = self.dataset_info['aa_encoder'] \
        ...
        self.inth_nf = len(self.inth_type_decoder)
        self.inthp_nf = len(self.inthp_type_decoder)
        self.x_dims = 3

        net_dynamics = EGNNDynamics(
            atom_nf=self.atom_nf,
            residue_nf=self.aa_nf,
            interh_nf=self.inth_nf,
            interhp_nf=self.inthp_nf,
            n_dims=self.x_dims,
    ...
    def get_ligand_and_pocket(self, data):
        ...
        interh = {
            'x': data['interh_coords'].to(self.device, FLOAT_TYPE),
            'one_hot': data['interh_one_hot'].to(self.device, FLOAT_TYPE),
            'size': data['num_interh_nodes'].to(self.device, INT_TYPE),
            'mask': data['interh_mask'].to(self.device, INT_TYPE)
        }
        interhp = {
            'x': data['interhp_coords'].to(self.device, FLOAT_TYPE),
            'one_hot': data['interhp_one_hot'].to(self.device, FLOAT_TYPE),
            'size': data['num_interhp_nodes'].to(self.device, INT_TYPE),
            'mask': data['interhp_mask'].to(self.device, INT_TYPE)
        }
        return ligand, pocket, interh, interhp

    def forward(self, data):
        #print('===== lightning ======')
        ligand, pocket, interh, interhp = self.get_ligand_and_pocket(data)
        #print('ligand', ligand)
        #print('pocket', pocket)
        #print('interh', interh)
        #print('interhp', interhp)
        ...
        kl_prior, log_pN, t_int, xh_lig_hat, alpha_t, info = \
            self.ddpm(ligand, pocket, interh, interhp, return_info=True)
    ...
    def sample_and_analyze_given_pocket(self, n_samples, dataset=None,
        ...
        for i in range(math.ceil(n_samples / batch_size)):
            ...
            ligand, pocket, interh, interhp = self.get_ligand_and_pocket(batch)
            ...
            xh_lig, xh_pocket, lig_mask, _ = self.ddpm.sample_given_pocket(
                pocket, interh, interhp, num_nodes_lig)
            #xh_lig, xh_pocket, lig_mask, _ = self.ddpm.sample_given_pocket(
    ...
    def sample_and_save_given_pocket(self, n_samples):
        ...
        xh_lig, xh_pocket, lig_mask, pocket_mask = \
            self.ddpm.sample_given_pocket(pocket, interh, interhp, num_nodes_lig)
    ...
    def sample_chain_and_save_given_pocket(self, keep_frames):
        ...
        chain_lig, chain_pocket, _, _ = self.ddpm.sample_given_pocket(
            pocket, interh, interhp, num_nodes_lig, return_frames=keep_frames)
